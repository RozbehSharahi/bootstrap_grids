...
4. OPTIONAL: Do `cp -i .docker/.env.example .docker/.env` before the next step if you need anything other than the defaults (TYPO3 13, PHP 8.4, HTTP_PORT 8080). Set `TYPO3=12` or `TYPO3=13`, `PHP=8.2`, `8.3`, or `8.4`, and `HTTP_PORT` for the host port. `TYPO3=` selects Docker templates and pins Composer to that core line. To switch TYPO3 version after a previous `.docker/bin/up`, run `.docker/bin/destroy` first. Otherwise `.docker/.env.example` will automatically be copied to `.docker/.env` if it doesn't already exist and you can skip this step.
5. OPTIONAL: Start Xdebug if you need to debug PHP code.
6. Run `.docker/bin/up`. The container copies templates and installs Composer dependencies for the selected TYPO3 line. The command waits up to five minutes for the container to become healthy. Then open [http://localhost:8080/typo3](http://localhost:8080/typo3) (or `http://localhost:<HTTP_PORT>/typo3`).
...
| -------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.docker/bin/up`                 | ✗              | Runs `.docker/bin/compose up -d --build --wait --wait-timeout 300`. Container start copies templates, runs Composer for `TYPO3=`, and waits until healthy.                                                                                                        |
...
| `.docker/bin/compose [command]`  | ✗              | Wrapper around Docker Compose used by all helper scripts: auto-creates `.docker/.env` from `.docker/.env.example` when missing, exports vars, prints selected `TYPO3`/`PHP`/`HTTP_PORT`, then runs `docker compose -f .docker/docker-compose.base.yml [command]`. |
