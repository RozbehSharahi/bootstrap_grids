...
_NOTE: PHP is capped at 8.4 for now. `gridelements` 13 declares `~8.2 || ~8.3 || ~8.4`, and this extension still supports TYPO3 12, whose Composer constraint is `^8.1` (no 8.5)._
...
| `.docker/bin/composer [command]` | ✓              | Runs `.docker/bin/compose exec app composer "$@"`. If `COMPOSER_IGNORE_PLATFORM_REQ` is set in `.docker/.env`, prepends `--ignore-platform-req=<value>`.                                                                                           |
